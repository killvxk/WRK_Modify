...

{
    ...
    /* 如果ExceptionRecord结构是空的，则构造一个局部的结构，并填充，再展开的过程中调用注册异常处理handler时会使用 */
    ...
    /* 如果TargetFrame被指定了，则设置EXCEPTION_UNWINDING,否则的话设置EXCEPTION_EXIT_UNWIND和EXCEPTION_UNWINDING */
    ...
    while (RegistrationPointer != EXCEPTION_CHAIN_END) {
        ...
        //
        /* 如果TargetFrame比当前frame在栈上位置低，则抛出STATUS_INVALID_UNWIND */
        ...
        /* 如果call frame不在指定的stack范围内，或者call frame未对齐，则raise异常STATUS_BAD_STACK */
        HighAddress = (ULONG)RegistrationPointer +
            sizeof(EXCEPTION_REGISTRATION_RECORD);
