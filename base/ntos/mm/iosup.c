...
Arguments:
    ...
    CacheType - Supplies MmNonCached if the physical address is to be mapped
                ...
                For I/O device registers, this is usually specified
                as MmNonCached.
    如果物理地址是要被映射成非缓存的，则指定MmNonCached
    如果物理地址需要被缓存，则指定MmCached
    如果物理地址需要被缓存同时write-Combined，用于video port driver作为一个buffer,则指定MmWriteCombined
    所有别的调用，应该指定MmUSWCCached. MmUSWCCached只有在PAT 可用时有效
    注意: 对于I/O 设备寄存器，通常指定为MmNonCached
Return Value:
    ...
    映射成功返回有效的虚拟地址，失败则NULL
Environment:

    Kernel mode, Should be IRQL of APC_LEVEL or below, but unfortunately
    callers are coming in at DISPATCH_LEVEL and it's too late to change the
    rules now.  This means you can never make this routine pageable.
    内核下使用，IRQL<=APC_LEVEL
--*/

{
    ...
    PageFrameIndex = (PFN_NUMBER)(PhysicalAddress.QuadPart >> PAGE_SHIFT);                          // 获取PFN 号

    IoMapping = (CSHORT) (!MI_IS_PFN (PageFrameIndex));                                             // 检测是否是有效的PFN号，如果是，则意味着是PFN映射，若不是，则是端口映射
    ...
    PointerPte = MiReserveSystemPtes ((ULONG)NumberOfPages, SystemPteSpace);                        // 从System PTE区域申请一个 PTE
    ...
    BaseVa = (PVOID)MiGetVirtualAddressMappedByPte (PointerPte);                                    // 获取该PTE所指向的虚拟地址
...
Routine Description:
    ...
    该函数查找PFN数据库来查找free, zeroed,或者Standby的页面来完成请求。
    它并不映射页，而只是分配它们，并把它们放到MDL中。期望调用来根据需求映射MDL。（意思是这有物理地址，但是并没有虚拟地址么？I Feel)
    ...
    该函数设计的目的是给AGP Driver用去获取指定范围的一段物理内存，由于硬件可能需要提供实质的性能提升，依赖在backing memory分配在哪里
    ...
    因为调用者可能使用这些页面用于非缓冲的映射，注意，绝对不要分配处于大页面中的页，避免产生TB对同一页的不一致情况， 由于多次翻译不同的属性
Arguments:
    ...
    指定获取下一个分配的物理地址的范围，从Low Address需要跳过的字节
