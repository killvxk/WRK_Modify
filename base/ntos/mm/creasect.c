...

{
    ...
    //
    /* 检测保护属性flag,调用该函数实现将用户提供的SectionPageProtection转化为内核使用的5bit 的PTE protect flag*/
    ...
    {
        /* FileHandle 或者 FileObject有效，不支持SEC_LARGE_PAGES */
        ...
        // be checked.
        //
        /* 文件FileHandle or FileObject 应该被提供。如果FileObject有效，则一定是来自文件系统，故文件大小不需要检查 */
        if (ARGUMENT_PRESENT(FileObject)) {
            ...
            if (File->SectionObjectPointer->DataSectionObject) 
            {
                    ...
                    (!ControlArea->u.Flags.BeingCreated)) 
                {
                    ...
                    // reference it.
                    //
                    /* ControlArea存在，并且并没有被deleted,则引用 */
        ...
        else 
        {
            ...
            // return an error.
            //
            /* 引用文件句柄的文件对象，校验该文件对象的SectionObjectPointer是否有效 */
        ...
        if (AllocationAttributes & SEC_IMAGE) 
        {
            ...
            //
            /* control area 常常只是一个占位的，真正的是通过MiCreateImageFileMap创建，同时被分配正确的大小，这个将在稍后释放 */
        ...
        //
        /* 如果这是一个user request,我们只是需要文件资源。i.e 不是来自缓存管理器或者文件系统的Call */
...
RecheckSegment:
        ...
        if (ControlArea != NULL) 
        {
            ...
            //
            /* A Segment 已经存在了对于 this file. 确认它不是正在处理或者是正在被创建 */
...

{
    ...
    *GlobalNeeded = FALSE;
    ...
    //
    /* 在UserGlobalList链表中寻找与file poiner下 ImageSectionObject中Session匹配的Control Area */
