...

{
    ...
    if (Phase == 0) {
        ...
        MmHighestUserAddress = (PVOID)(KSEG0_BASE - 0x10000 - 1);							// 初始化全局变量，用户地址空间的大小
        MmUserProbeAddress = KSEG0_BASE - 0x10000;
        MmSystemRangeStart = (PVOID)KSEG0_BASE;

#endif

        MiHighestUserPte = MiGetPteAddress (MmHighestUserAddress);							// 初始化最大用户地址空间的PTE   MiHighestUserPte
        MiHighestUserPde = MiGetPdeAddress (MmHighestUserAddress);							// 初始化最大用户地址空间的PDE   MiHighestUserPde
        ...
        DefaultSystemViewSize = MM_SYSTEM_VIEW_SIZE;										// 系统视图大小,x32位16M，x64位104M
        MmSessionSize = MI_SESSION_SPACE_DEFAULT_TOTAL_SIZE;								// 由N个宏相加，详见宏。
        SessionEnd = (ULONG_PTR) MM_SESSION_SPACE_DEFAULT_END;								// c0000000之下（低地址空间）
...
#define MM_MB_MAPPED_BY_PDE (MM_VA_MAPPED_BY_PDE / (1024*1024))								// ????
        ...
        if (MmSystemViewSize < (MM_VA_MAPPED_BY_PDE / (1024*1024))) {						// 最小的系统视图大小不能小于一个PDE虚拟地址空间的大小，兆为单位
            MmSystemViewSize = DefaultSystemViewSize;										// MM_VA_MAPPED_BY_PDE代表16M大小
        }
        else {
            ...
            if (MmVirtualBias == 0) {														// 是否需要调整
                ...
                MmSystemViewSize = MI_ROUND_TO_SIZE (MmSystemViewSize,
                                                     MM_MB_MAPPED_BY_PDE);					// 根据系统视图大小根据四字节大小对齐
                ...
                SystemViewMax = (MI_SESSION_SPACE_MAXIMUM_TOTAL_SIZE) / (1024*1024);		// 最大系统视图空间大小为(c0000000-a0000000)/1M = 200M
                ...
                SystemViewMax -= (MmSessionSize / (1024*1024));								// 至少为标准的默认会话留一定空间
#endif
                ...
                if ((MmSystemViewSize > SystemViewMax) ||
                    ...
                    MmSystemViewSize = SystemViewMax;										// 系统视图大小
                }
...
#else
        HydraSpaceUsedForSystemViews = MmSystemViewSize;									// 超空间使用的大小
#endif
        MiSessionImageEnd = SessionEnd;														// 会话映像终点0xc0000000
        ...
        if (MmVirtualBias == 0) {															// 需不需要对齐 0 不需要 1 需要

            if (MmSessionImageSize < MM_MB_MAPPED_BY_PDE) {									// 系统映射大小不能小于一个PDE代表的大小(4M)
                MmSessionImageSize = MI_SESSION_DEFAULT_IMAGE_SIZE;							// 设置会话映像大小默认8M
            }
            else {
                ...
                MmSessionImageSize = MI_ROUND_TO_SIZE (MmSessionImageSize,
                                                        MM_MB_MAPPED_BY_PDE);				// 按照PDE(4M)大小调整
                ...
                if ((MmSessionImageSize > HydraImageMax) ||									// HydraImageMax 最大的会话映射大小
            ...
            MiSessionImageStart = SessionEnd - MmSessionImageSize;							// 之上同理初始化会话映像区的
            ...
            MiSessionViewStart = SessionEnd - MmSessionImageSize - MI_SESSION_SPACE_WS_SIZE - MI_SESSION_SPACE_STRUCT_SIZE - MmSessionViewSize;			// 之上同理初始化会话视图区域的

            //
            // The session view start and size has been established.
            //
            // Now initialize the session pool start and size which lies								// 初始化会话POOL 的
            ...
            MiSessionPoolStart = MiSessionPoolEnd - MmSessionPoolSize;									// 之上是初始化会话POOL的
			...
			/* 不需要对齐内存的话，关于会话空间的初始化直接来这,包括MmSessionImage, MmSessionView , MmSessionPool */
            //
        ...
        MaximumSystemCacheSize = (MM_SYSTEM_CACHE_END - (ULONG_PTR) MmSystemCacheStart) >> PAGE_SHIFT;				// 系统缓存大小，0xC1000000-0xE1000000
        ...
        MiSessionLastPte = MiGetPteAddress ((PVOID)MiSessionSpaceEnd);									// 初始化一些全局的视图变量
        ...
        NumberOfPages = MiPagesInLoaderBlock (LoaderBlock, IncludeType);						// 从LoaderBlock的MemoryList获取系统页的数量，根据它来判断系统默认PTE的数量
        ...
        SystemPteMultiplier = 0;														// 表明统计系统PTE数量是，是否根据SystemPteMultiplier*NumberOfPages来获得
...
#else
            if (NumberOfPages < MM_MEDIUM_SYSTEM) {											// 如果是中型系统的话，设置系统PTE数量默认为7000
                ...
                if (NumberOfPages > 8192) {
                    ...
                    if (ExpMultiUserTS == TRUE) {
                        MmNumberOfSystemPtes = MM_MAXIMUM_SYSTEM_PTES;						// 最大50000个
                    }
                }
            }
#endif
...
#else
            MmNumberOfSystemPtes = MM_MAXIMUM_SYSTEM_PTES;									// 注册表若明示需要最大的PTE，之后需要分配。MiRequestedSystemPtes表明是否需要.
#endif
        }
        ...
        if (SystemPteMultiplier != 0) {
            if (NumberOfPages * SystemPteMultiplier > MM_MAXIMUM_SYSTEM_PTES) {
                ...
                MmNumberOfSystemPtes = (ULONG)(NumberOfPages * SystemPteMultiplier);		// 放大倍数
            }
#if defined (_WIN64)
        ...
        if (MmHeapSegmentReserve == 0) {
            MmHeapSegmentReserve = 1024 * 1024;									// 保留的堆段默认为1M
        }

        if (MmHeapSegmentCommit == 0) {
            MmHeapSegmentCommit = PAGE_SIZE * 2;								// 默认的提交堆段默认为PAGE_SIZE*2
        }

        if (MmHeapDeCommitTotalFreeThreshold == 0) {
            MmHeapDeCommitTotalFreeThreshold = 64 * 1024;						// 最大的堆解除提交上限为64K
        }

        if (MmHeapDeCommitFreeBlockThreshold == 0) {
            MmHeapDeCommitFreeBlockThreshold = PAGE_SIZE;						// 最大的一次性提交的堆解除为一个页大小
        }
        ...
        if ((MmVerifyDriverBufferLength != (ULONG)-1) ||
            ((MmSpecialPoolTag != 0) && (MmSpecialPoolTag != (ULONG)-1))) {
            MmNumberOfSystemPtes += MM_SPECIAL_POOL_PTES;						// 系统PTE数量继续增加，若MmVerifyDriverBufferLength不为-1,或者SpecailPoolTage标志
        }

        MmNumberOfSystemPtes += BBTPagesToReserve;								// BBTPagesToReserve ????
        ...
        MmTotalCommitLimit = _2gb / PAGE_SIZE;											// 设置初始提交页的最大限制，要足够大，使初始化POOL分配的时候能确保成功
        ...
        if (MmAllocationFragment == 0) {
            ...
            MmAllocationFragment = MM_ALLOCATION_FRAGMENT;								// 设置AllocationFragment ，不知干嘛用它？？？？？？？？？
#if !defined (_WIN64)
            ...
            AutosizingFragment = FALSE;														// 设置AllocationFragment ，不知干嘛用它？？？？？？？？
        ...
        MiInitializeCacheOverrides ();														// 初始化缓存重写机制，x64需要，x32不用。未读
