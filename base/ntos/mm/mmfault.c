...

{
    ...
    /* 如果地址不合法，则返回错误，主要检查地址的每一个标志位，针对x86，这个宏都是TRUE */
    ...
    /* 以下是IRQL<=APC_LEVEL的情况 */
    ApcNeeded = FALSE;
    WsIrql = MM_NOIRQL;

    if (VirtualAddress >= MmSystemRangeStart) 
    {
        /* 这是一个发生在系统地址空间的错误。用户模式的访问不允许 */
        //
...
#if (_MI_PAGING_LEVELS >= 4)        // 大于4级页表之上的情况
...
#if (_MI_PAGING_LEVELS >= 3) /* 大于3级页表之上的情况 */
        ...
        if (PointerPde->u.Hard.Valid == 1)                      // 如果PDE有效
        {

            if (MI_PDE_MAPS_LARGE_PAGE (PointerPde))            // 验证是Pde.u.Har.LargePage == 1
            {
        ...
        if (TempPte.u.Hard.Valid == 1) 
        {
            ...
            //
            /* 检查虚拟地址是否落在系统全局设定的PTE映射保护区，若是，则直接由该函数处理 */
            if (!IsListEmpty (&MmProtectedPteList)) {
            ...
            // and handled below.
            //
            // 会话地址空间的错误不能提前exit,因为可能是copy-on-write, 必须之后检测和处理
        ...
        /* 以上都是TempTte.u.Hard.Valid == 1的一些权限啊等等一类的情况 */
#if (_MI_PAGING_LEVELS < 3)
        ...
        if (MI_IS_PAGE_TABLE_OR_HYPER_ADDRESS (VirtualAddress)) 
        {
            ...
            /* 调用该函数处理不同进程空间PTE页表不一致的问题，若该函数不能处理，则当做用户地址空间的错误来处理 */
