...

{
    ...
    if (InitializationPhase == 1) {
        ...
        // with large pages.
        //
		/* KeFeatureBits 在初始化内核时由PCRB中的成员feature赋值 */
	...
	/* 消除注册表对栈的最大范围的设定，注意注册表默认大小单位为1K。同时注意，注册表设定不包括保护页，为了减少管理员了解这个机制，我们默认从他们请求中扣掉一个页 */
    if (MmLargeStackSize > (KERNEL_LARGE_STACK_SIZE / 1024)) {
        ...
        MmLargeStackSize -= PAGE_SIZE;														/* 扣掉一个保护页的大小 */
	...
	/* 检测处理器是否支持全局页位，若支持，则设置到template kernel PTE and PDE entries ???? */
    ...
    CurrentProcess->Pcb.DirectoryTableBase[0] = DirBase;								// 设置页目录（物理地址)
    KeSweepDcache (FALSE);									// 空
	...
	/* 初始化低2GB空间 */
    MiZeroMemoryPte (PointerPde, LastPte - PointerPde);									// idle 进程不用低2GB空间
	...
	/* 自动启用特定的IRQL，禁用使用大页面映射Kernel ,这样我们可以catch Kernel and hal code */
