...

{
    ...
    QuotaCharge = 0;
    /* CapturedBase 为NULL 或者 AllocationType 属性中呆着MEM_RESERVE标志的情况 */
    if ((CapturedBase == NULL) || (AllocationType & MEM_RESERVE)) 
    {
        ...
        else 
        {
            ...
            else 
            {
                ...
                //
                /* StartingAddress 其实地址64K对齐，也就是说0x10 倍VPN  */
...
Inserted:
    ...
    //
    /* 这是MEM_COMMIT的情况。Commit 之前保留的页。 注意，这些页可能是private或者是一个section */
    ...
    /* 如果FoundVAD的CommitCharge已经达到MM_MAX_COMMIT值，则错误 */
    ...
    /* FondVad 不是 PrivateMemory的情况。也即FoundVad VadFlags中的PrivateMemory 为0  */
