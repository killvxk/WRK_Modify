...

{
    ...
    //  items.
    //
    /* 分配和初始化工作线程，插入到CcIdleWorkerThreadList结构上 */
    ...
    //
    /* 初始化LazyWrite的 Scan Dpc 和 Timer */
    ...
    //
    /* 对每一个处理器初始化其上的lookaside list中的LookasideTwilightList链表 */
...
Routine Description:
    ...
    该例程是给文件系统调用的。初始化数据缓存的cache map结构。每次文件打开时创建，同时没有指定NO_INTERMEDIATE_BUFFERING标志
Arguments:
...
restart:
        ...
        //
        /* 如果这个文件有FO_SEQUENTIAL_ONLY 标志，则在SharedCacheMap中设置 */
