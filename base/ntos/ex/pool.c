...

{
    ...
    LockHandle.OldIrql = 0;                                                     // 为了w4编译过去而初始化

    /* ExpPoolFlags 包括的的几种情况 */
    if (ExpPoolFlags & (EX_CHECK_POOL_FREES_FOR_ACTIVE_TIMERS |
                        EX_CHECK_POOL_FREES_FOR_ACTIVE_WORKERS |
                        ...
                        EX_SPECIAL_POOL_ENABLED)) 
    {
        ...
        if (!PAGE_ALIGNED(P)) {
            ...
            PoolType = (Entry->PoolType & POOL_TYPE_MASK) - 1;                         // PoolType 的获取是与一个POOL_TYPE_MASK 还减1

            CheckType = PoolType & BASE_POOL_TYPE_MASK;                                 // CheckType是在此基础上，继续与上基本类型
            ...
            NextEntry = (PPOOL_HEADER)((PPOOL_BLOCK)Entry + Entry->BlockSize);          // 校验内存上大小是否一致
            ...
            NumberOfBytes = (SIZE_T)Entry->BlockSize << POOL_BLOCK_SHIFT;               // 真正的大小

            if (ExpPoolFlags & EX_VERIFIER_DEADLOCK_DETECTION_ENABLED) {                // 死锁检测
                VerifierDeadlockFreePool (P, NumberOfBytes - POOL_OVERHEAD);
            }

            if (Entry->PoolType & POOL_VERIFIER_MASK) {                                 // VERIFIER MASK检测
    ...
    //
    /* 如果Entry是页对齐的，那么释放该块到页对齐的池。否则，按照正常释放到allocation lists */
    if (PAGE_ALIGNED(P)) 
    {

        PoolType = MmDeterminePoolType (P);             // 通过地址所在范围判断是属于哪种池
        ...
        if (PoolType == PagedPoolSession) 
        {
            PoolDesc = ExpSessionPoolDescriptor;        // Session 池就使用SessionPool 描述符ExpSessionPoolDescriptor
        }
        else
        {

            PoolDesc = PoolVector[PoolType];            // 否则就从PoolVector中取出合适的池描述符

            if (CheckType == NonPagedPool)              // 检测NonPagedPoolSession的情况
            {
        ...
        Tag = ExpFindAndRemoveTagBigPages (P, &BigPages, PoolType); // 大页面的检测
    ...
    //
    /* 对齐entry地址到pool allocation 边界 */
    Entry = (PPOOL_HEADER)((PCHAR)P - POOL_OVERHEAD);               // 上面一个是根据ExPoolFlags进行相关检测的情况，一个是针对大页面的释放，此处才是通常情况下的执行

    BlockSize = Entry->BlockSize;

    EntryPoolType = Entry->PoolType;

    PoolType = (Entry->PoolType & POOL_TYPE_MASK) - 1;              // 所有的Pool Type都减1，看来在分配时都是先加1
    ...
    Tag = Entry->PoolTag;
    if (Tag & PROTECTED_POOL)                                   // 如果有保护池的标志，则去掉
    {
        Tag &= ~PROTECTED_POOL;
    }
    ...
    NextEntry = (PPOOL_HEADER)((PPOOL_BLOCK)Entry + BlockSize);         // 在内存上校验是否有被破坏
    ...
    PoolDesc = PoolVector[CheckType];                               // 从PoolVector中取得池描述符，丫的。

    MARK_POOL_HEADER_FREED (Entry);

    if (EntryPoolType & SESSION_POOL_MASK)                          // Session Pool 的情况
    {
    ...
    else 
    {
        ...
        if (CheckType == PagedPool) 
        {
            ...
            PoolDesc = ExpPagedPoolDescriptor[DECODE_POOL_INDEX(Entry)];            // 在这里变了
        }
        else 
        {
            ASSERT ((DECODE_POOL_INDEX(Entry) == 0) || (ExpNumberOfNonPagedPools > 1));
            if (ExpNumberOfNonPagedPools > 1) 
            {
                PoolDesc = ExpNonPagedPoolDescriptor[DECODE_POOL_INDEX(Entry)];     // 看来默认都是PagedPool与NonPagedPool都是从这里获取池描述符信息的
            }
        }
    }
    ...
    ExpRemovePoolTrackerInline (Tag,                                                // 去掉 Pool Tracker 表中的相关信息
    ...
    if ( EntryPoolType & POOL_QUOTA_MASK )                                          // 设置配额的一些相关信息
    {
    ...
    if (((EntryPoolType & SESSION_POOL_MASK) == 0) ||                               // 如果Pool block is a small block ，最大32*8(0x20*8)个字节，则释放到Prcb相应的LookAside List中
        (CheckType == NonPagedPool)) 
    {
        ...
        if ((BlockSize <= POOL_SMALL_LISTS) && (USING_HOT_COLD_METRICS == 0)) 
        {
            ...
            if (CheckType == PagedPool) 
            {
                ...
                if (KeNumberNodes > 1) 
                {
                    if (Prcb->ParentNode->Color != PoolDesc->PoolIndex - 1)                     // 这个判断是为何？？
                    {
                        goto NoLookaside;
                    }
                }
    ...
    else 
    {
        ...
        if (BlockSize <= ExpSessionPoolSmallLists)                      // 将内存块释放到session lookaside list中
        {
            ...
            LookasideList = (PGENERAL_LOOKASIDE)(ULONG_PTR)(ExpSessionPoolLookaside + BlockSize - 1);           // 这是个全局变量ExpSessionPoolLookaside
...
NoLookaside:
    ...
    if (ExpPoolFlags & EX_DELAY_POOL_FREES)                                                         // 如果带有延迟释放的操作，同时这个链表已满，执行ExDeferredFreePool操作
    {
    ...
    if (PAGE_END(NextEntry) == FALSE)                                                               // 合并内存块的一个操作
    {
    ...
    if (Entry->PreviousSize != 0)                                                                           // 前一个内存块的合并
    {
    ...
    if (PAGE_ALIGNED(Entry) &&                                                              // 如果合并的大小已经到了一个页，则归还内存管理器
...

{
    ...
    LockHandle.OldIrql = 0;                                                 // w4 级编译需要初始化
    ...
    LOCK_POOL (PoolDesc, LockHandle);                                       // 区别不同的池类型，加不同的锁，提升到对应的中断请求级别上

    if ( PoolDesc->PendingFrees == NULL )                                   // PendingFrees 为空就返回
    {
        UNLOCK_POOL (PoolDesc, LockHandle);
        return;
    }
    ...
    do                                                                      // 把链表上的每一项处理
    {
        ...
        do 
        {
            ...
            NextEntry = (PPOOL_HEADER)((PPOOL_BLOCK)Entry + Entry->BlockSize);                      // 获取内存紧邻的下一个Entry
            if (PAGE_END(NextEntry) == FALSE)                                                       // 检查页对齐
            {
                if (NextEntry->PoolType == 0)                                                       // 是NonPagedPool，并且空闲
                {
                    Combined = TRUE;
                    ...
                    Entry->BlockSize = Entry->BlockSize + NextEntry->BlockSize;                     // 合并后一个内存块
                }
            }
            ...
            {
                NextEntry = (PPOOL_HEADER)((PPOOL_BLOCK)Entry - Entry->PreviousSize);
                if (NextEntry->PoolType == 0)                                                       // 前一个块是否也空闲，空闲就合并，为什么以0来判断。。。
                {
                    Combined = TRUE;
                    ...
                    NextEntry->BlockSize = NextEntry->BlockSize + Entry->BlockSize;                 // 合并前一个内存块
                    Entry = NextEntry;
                }
            }

            if (PAGE_ALIGNED(Entry) &&
                (PAGE_END((PPOOL_BLOCK)Entry + Entry->BlockSize) != FALSE))                         // 整页数加1，这是要归还给内存管理器
            {
            ...
            else                                                                                    // 插到对应的链表中去
            {
                ...
                if (Combined != FALSE)                                                              // 如果合并过内存块，就要调整内存中后一个块的PreviousSize
                {
            ...
            if (NextSingleListEntry == LastEntry)                                                   // 单链表通过检查下一个是否为NULL，检查是否到尾
            {
                break;
            }
            SingleListEntry = NextSingleListEntry;                                                  // 处理链表中的下一个
    ...
    UNLOCK_POOL (PoolDesc, LockHandle);                                                             // 解锁

    if (WholePages != NULL)                                                                         // 归还整页的内存
    {
        ...
        do 
        {
            ...
            PERFINFO_FREEPOOLPAGE(CheckType, PoolIndex, Entry, PoolDesc);                           // 性能分析
            MiFreePoolPages( Entry );
            Entry = NextEntry;
        } while (Entry != NULL);
    }
