...
{
    /* 创建一个handle table entry 在指定的handle table中，同时返回指定entry的handle */
    EXHANDLE Handle;
    PETHREAD CurrentThread;
    PHANDLE_TABLE_ENTRY NewHandleTableEntry;
    ...
    /* 分配一个新的handle table entry, 获取其handle value值 */
...
{
    /* 映射一个句柄指向一个对应的句柄表项，如果映射成功，则当返回时，句柄表项is locked */
    EXHANDLE LocalHandle;
...

{
    ...
    /* 又在PagedPool 上分配了0x1000(一个页面4K)大小的空间 */
    ...
    /* TableCode指向新分配的HandleTableTable */
    ...
    /* 给第一个HANDLE_TABLE_ENTRY项结构中NextFreeTableEntry赋值EX_ADDITIONAL_INFO_AIGNTURE，为了在以后使用表明这是一个特殊信息的entry  */
    ...
    if (DoInit) {
        ...
        for (i = 1; i < LOWLEVEL_COUNT - 1; i += 1) {
            ...
            HandleEntry->Value = 0;
            HandleEntry->NextFreeTableEntry = (i+1)*HANDLE_VALUE_INC;   /* 下一个个空闲句柄的索引 */
            HandleEntry++;
        }
        HandleEntry->Value = 0;
        HandleEntry->NextFreeTableEntry = 0;

        HandleTable->FirstFree = HANDLE_VALUE_INC;                  /* 第一个空闲句柄的索引是4 */

    }
    

    HandleTable->NextHandleNeedingPool = LOWLEVEL_COUNT * HANDLE_VALUE_INC; /* 指定下一个句柄表扩展的起始索引 */
