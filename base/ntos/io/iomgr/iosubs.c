...
Routine Description:
    ...
    该例程用来完成一个IO请求。通常是在一个驱动在其DPC例程中调用该函数来完成最终的IRP的结束。
    该例程主要实现的功能如下:
        ...
        1.  首先检查是否IRP 栈已经遍历完了。如果没有，则设置IRP栈指针位下一栈，
            之后检查其是否有完成例程或者取消例程需要调用，若有，调用，直到没有
            例程调用了，或者遍历完了栈。（该情况其实就是在你返回了STATUS_MORE_PROCESS_REQUIRED 后，你在你的当前站里继续调用IoCompelteRequest来向上传递的情况。）
        ...
        2.  检查该IRP是否是一个关联IRP。如果是， 则 Master IRP上的count数量减少。
            如果Master IRP上的count数量减少到0了，则像正常IRP一样，调用IoCompleteRequest来结束Master IRP.
            如果非0 ，则正常释放该IRP（ 这是处于从IRP角度来看的）
        ...
        3.  如果这是一个Paging I/O 或者是一个关闭的操作，则只要写该IRP的I/O status block同时
            设置信号为有信号状态，同时减少该信号的计数。 如果这是个Paging IO, 则最后在释放该IRP。
        ...
        5.  检查是否这个完成过程可以退出。如果可以， 则该例程就简单的退出。把最终的
            完成操作丢给最原始的 创造该IRP的线程.
        ...
        6.  最终的结束例程被以一个特殊内核APC的方式插入到了最初请求线程中。
...

{
    ...
    // the packet.
    //
    /* 断言该IRP的状态一定不是STATUS_PENDING,如果在结束一个IRP时，状态还是保留在Pending，说明一定是忘了设置状态为最新的状态*/
    ...
    /* 循环检查每一个IRP Stack */
    for (stackPointer = IoGetCurrentIrpStackLocation( Irp ), Irp->CurrentLocation++, Irp->Tail.Overlay.CurrentStackLocation++;
         ...
         stackPointer++, Irp->CurrentLocation++, Irp->Tail.Overlay.CurrentStackLocation++ ) 
    {
            ...
            ) 
        {
            ...
            /* 零化irp stack （好像没有零化major function ) */
        ...
        {
            /* 如果不调用对应的完成例程，继续标记为SL_PENDING_RETURNED */
    ...
    //
         ...
         /* 根据FLAGS标志位判断是否是associate IRP ,若是则减少主IRP的计数 */
    if (Irp->Flags & IRP_ASSOCIATED_IRP) 
    {
        ...
        count = IopInterlockedDecrementUlong( 
        ...
        if ( count == 1 )                       // 检测是否只剩余自己了，若是则调用完成IRP
        {
    ...
    /* 检查我们是否有一个name function???, 如果是的话，我们给变形reparse point 做好准备？？*/
    ...
    /* 做一个最终的检查去判断现在要关闭的是否是一个同步I/O的操作对于原始请求者来说，如果是，则一个最优的路径通过I/O completion可以被采取 */
    ...
    /* 最后初始化该IRP作为APC的 结构，然后插入特殊的APC到目标线程*/
    thread = Irp->Tail.Overlay.Thread;
