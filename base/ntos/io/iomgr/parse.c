...

{
    ...
    if (Length < sizeof( OBJECT_NAME_INFORMATION ))                                    // 如果请求的长度小于sizeof(OBJECT_NAME_INFORMATION)
    {
    ...
    buffer = ExAllocatePoolWithTag( PagedPool, Length, '  oI' );                      // 分配参数给定大小的Buffer
    ...
    fileObject = (PFILE_OBJECT) Object;                                              // 转换为文件对象
    deviceNameInfo = (POBJECT_NAME_INFORMATION) buffer;                              // 转义为OBJECT_NAME_INFORMATION

    if (UseDosDeviceName)                                                            // 是否转换为Dos名
    {
        if (fileObject->DeviceObject->DeviceType == FILE_DEVICE_NETWORK_FILE_SYSTEM) // 如果为网络设备文件系统
        {
            ...
            *p = '\\'; // Start with a '\' as RDR does not return the extra
            deviceNameInfo->Name.Buffer = p;                                        // 网络设备文件系统设备名的特殊处理

        } 
        else 
        {
            status = IoVolumeDeviceToDosName( fileObject->DeviceObject, &deviceNameInfo->Name );            // 正常情况下的转换
        ...
        if (!NT_SUCCESS(status))                                                  // 如果转换Dos名字失败，至少返回设备名
        {
        ...
        {
            dosLookupSuccess++;                                                  // 计数加1
        }
    } 
    else                                                                         // 如果 不转换Dos名，则直接查询
    {
        status = ObQueryNameString( (PVOID) fileObject->DeviceObject,
                                deviceNameInfo,
                                Length,
    ...
    p = (PWSTR) (ObjectNameInfo + 1);                                  // 指向Buffer
    ...
    try 
    {
        ...
        if (UseDosDeviceName && dosLookupSuccess)                       // 转换了名字，并且成功
        {
            ...
            RtlCopyMemory( ObjectNameInfo,                                                  // 为什么不直接拷贝，要分开拷贝啊
                           deviceNameInfo,
                           BaseCopyLength );
            RtlCopyMemory( p,
        ...
        ObjectNameInfo->Name.Buffer = p;                                        // p 值赋值给ObjectNameInfo->Name.Buffer?
        ...
        /* 如果设备已经满了，标注和继续获取文件名字长度， 我们期望返回请求的长度，对于整个结果 */
        ...
        /* 重新设置buffer来获取名字中文件名部分，同时计算调用者buffer的剩余的长度，注意，在以下的计算中，有两种情况 */
        /*  上面查询的返回长度的设备名字在结尾处需要包括一个空字符，这个应该包括在计算范围内*/
        /* 对象名字信息的大小假设应该大于文件名字偏移，在一个文件信息的buffer中。因此，buffer变量的长度可以设置为剩下的长度加上至少4个 */
        fileNameInfo = (PFILE_NAME_INFORMATION) buffer;                      // 这是要复用buffer ?
