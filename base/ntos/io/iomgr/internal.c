...
#define SynchronousIo( Irp, FileObject ) (  \
    ...
    /* 判断全局标志IopVerifierOn来检测是否结束当前IRP */
    ...
    /* 检测IRP尚未被结束，通过判断IoStatus.Status是否为-1 */
    ...
    /* 检查是否是BufferIo，使得话拷贝内容到用户指定的缓冲区，同时释放系统默认的缓冲区 */
    ...
    /* 检查是否I/O操作彻底完成了。若是，则正常处理，若不是，处理掉所有东西 */
        ...
        !SynchronousIo( irp, fileObject )))
    {
...
#else
            /* 将IoStatus中的信息拷贝到UserIo status block上 */
            irp->UserIosb->Information = irp->IoStatus.Information;
            KeMemoryBarrierWithoutFence ();
            *((volatile NTSTATUS *) &irp->UserIosb->Status) = irp->IoStatus.Status;
#endif  /*_WIN64 */
        ...
        /* 看用户是否设置了一个event 需要被设置为有信号状态。如果是，设置；否则，设置文件对象中的event为有信号 */
        if ( irp->UserEvent ) 
        {
        ...
        /* 这是一个正常的I/O，更新进程的transfer count */
        if (!(irp->Flags & IRP_CREATE_OPERATION)) 
        {
        ...
        /* 如果调用者请求了一个APC，那么queue it 。如果没有，则只是简单的释放掉该包 */
#ifdef  _WIN64
        //
        ...
        else if ( port && irp->Overlay.AsynchronousParameters.UserApcContext ) 
        {
            ...
            //
            /* 如果一个完成上下文关联该I/O 操作，则给这个端口发送消息。标记完成例程 */
