...
typedef struct _KPROCESS {
    ...
    DISPATCHER_HEADER Header;			/* 分发器头，表明进程是一个分发器对象，可被等待 *[BAI]*/
    LIST_ENTRY ProfileListHead;			/* 当该进程参与性能分析时，作为一个节点加入到全局性能分析表中(内核全局变量KiProfileListHead) *[BAI]*/

    //
    // The following fields are referenced during context switches.
    //

    ULONG_PTR DirectoryTableBase[2];	/* 该进程的页目录表地址(PFN号)和Hyper Space页目录表地址 *[BAI]*/

#if defined(_X86_)

    KGDTENTRY LdtDescriptor;			/* 该进程的LDT描述符表 *[BAI]*/
    KIDTENTRY Int21Descriptor;			/* 允许通过int 21调用Dos功能 *[BAI]*/
    USHORT IopmOffset;					/* I/O Privilege Map 偏移 *[BAI]*/
    UCHAR Iopl;							/* 当前进程的I/O Privilege Level *[BAI]*/
    BOOLEAN Unused;
    ...
    volatile KAFFINITY ActiveProcessors;	/* 当前进程在哪些处理器上运行 *[BAI]*/

    //
    // The following fields are referenced during clock interrupts.
    //

    ULONG KernelTime;						/* 在内核态运行时间 等于所有子线程该值的和，每一个线程退出时更新该值*[BAI]*/
    ULONG UserTime;							/* 在用户态运行时间 等于所有子线程该值的和，每一个线程退出时更新该值*[BAI]*/

    //
    // The following fields are referenced infrequently.
    //

    LIST_ENTRY ReadyListHead;				/* 双向链表的表头，记录了该进程中处于就绪状态但尚未添加到全局就绪链表的线程，*/
											/* 当一个进程被换入时，该链表中的所有节点都会被加入到全局就绪链表中，该节点每一项指向一个KTHREAD结构的WaitListEntry域的地址 *[BAI]*/
    SINGLE_LIST_ENTRY SwapListEntry;		/* 当进程换入唤出时，通过此域将该进程加入到KiProcessOutSwapListHead/KiProcessInSwapListHead链表中 *[BAI]*/

#if defined(_X86_)

    PVOID VdmTrapcHandler;					/* 处理Ctrl-C中断的例程，仅在VDM环境下运行16未程序用 *[BAI]*/
    ...
    LIST_ENTRY ThreadListHead;				/* 当前进程的所有线程，一个线程创建后会加入，消亡后会删除 *[BAI]*/
    KSPIN_LOCK ProcessLock;					/* 操作当前进程数据成员时的自旋锁 *[BAI]*/
    KAFFINITY Affinity;						/* 表示该进程的线程可以在哪些处理器上运行 *[BAI]*/
...
#define KPROCESS_DISABLE_QUANTUM_BIT 2
    ...
    union {
        struct {
            LONG AutoAlignment : 1;		/* 该进程的内存访问对齐设置，会继承到线程中 *[BAI]*/
            LONG DisableBoost : 1;		/* 线程调度提升优先级用 *[BAI]*/
            LONG DisableQuantum : 1;	/* 线程调度提升优先级用 *[BAI]*/
    ...
    SCHAR BasePriority;					/* 该进程中线程的基本优先级，所有线程都会继承 ,此值和下值为线程调度用 *[BAI]*/
    SCHAR QuantumReset;					/* 该进程中某一个线程的基本时限(Quantum)值,现在Windows操作系统该值为6 *[BAI]*/
    UCHAR State;						/* 说明一个进程是否在内存中或者在换入换出过程中 *[BAI]*/
    UCHAR ThreadSeed;					/* 为每一个线程指定理想的处理器，给一个线程指定后，即更新值，为下一个线程准备，在多处理器环境下，每个线程都有优先选择的处理器 *[BAI]*/
    UCHAR PowerState;					/* 记录电源状态 *[BAI]*/
    UCHAR IdealNode;					/* 为一个进程选择优先的处理器节点，进程初始化时设定。 *[BAI]*/
    BOOLEAN Visited;					/* WRK中未使用，尚不清楚 *[BAI]*/
    union {
        KEXECUTE_OPTIONS Flags;
        UCHAR ExecuteOptions;			/* 设置一个进程的内存执行选项，为了支持NX(No-Execute，内存不可执行) *[BAI]*/
    };
    ...
    ULONG_PTR StackCount;				/* 所有位于内存中线程栈的个数 *[BAI]*/
    LIST_ENTRY ProcessListEntry;		/* 将当前系统中所有具有活动线程的进程串成一个链表，表头为KiProcessListHead *[BAI]*/
...
typedef struct _KTHREAD {
    ...
    DISPATCHER_HEADER Header;				/* 分发器对象 *[BAI]*/
    LIST_ENTRY MutantListHead;				/* 该线程拥有的所有突变体对象(mutant)，一旦被某个线程等到，则归该线程所有，加入到此链表中来 *[BAI]*/
    ...
    PVOID InitialStack;						/* 原始栈的高地址 *[BAI]*/	
    PVOID StackLimit;						/* 原始栈的低地址 *[BAI]*/
    PVOID KernelStack;						/* 栈真正的开始地址，相比较InitialStack较低，二者之间记录了浮点处理器保存区和一个异常陷进帧(KTRAP_FRAME_LENGTH+sizeof(FX_SAVE_AREA)) *[BAI]*/

    KSPIN_LOCK ThreadLock;					/* 线程自旋锁，保护线程数据成员 *[BAI]*/

    union {
        KAPC_STATE ApcState;				/* 指定了一个APC信息，其中的Process 域指向了该线程所属进程的KPROCESS结构 *[BAI]*/
        struct {
            UCHAR ApcStateFill[KAPC_STATE_ACTUAL_LENGTH];
            BOOLEAN ApcQueueable;			/* 是否可以插入APC *[BAI]*/
            volatile UCHAR NextProcessor;	/* 处理器调度的选择 *[BAI]*/
            volatile UCHAR DeferredProcessor;
            UCHAR AdjustReason;				/* 优先级的调整 *[BAI]*/
            SCHAR AdjustIncrement;
        };
    };

    KSPIN_LOCK ApcQueueLock;				/* 保护APC队列自旋锁 *[BAI]*/

#if !defined(_AMD64_)

    ULONG ContextSwitches;					/* 记录该线程进行了多少次环境切换 *[BAI]*/
    volatile UCHAR State;					/* 当前线程状态,详情参考KTHREAD_STATE *[BAI]*/
    UCHAR NpxState;							/* 浮点处理器的状态,不讨论 *[BAI]*/
    KIRQL WaitIrql;							/* 当WaitNext为TRUE时，记录原先的IRQL值 *[BAI]*/
    KPROCESSOR_MODE WaitMode;				/* 当线程等待时的处理器模式 *[BAI]*/

#endif

    LONG_PTR WaitStatus;					/* 记录了等待的结果状态 *[BAI]*/
    union {
        PKWAIT_BLOCK WaitBlockList;			/* 十字链表，记录该线程正在等待哪些对象，某个对象有多少个线程在等待 *[BAI]*/
        PKGATE GateObject;					/* 门对象特殊考虑，记录要等待的门对象，不会和其他对象同时发生 *[BAI]*/
    };

    BOOLEAN Alertable;						/* 若该线程处于正在等待状态时，若为TRUE，则表示可以被唤醒 *[BAI]*/
    BOOLEAN WaitNext;						/* 当为TRUE时，说明该线程马上要调用一个内核等待函数，与WaitIrql一起使用 *[BAI]*/
    UCHAR WaitReason;						/* 只是记录等待理由，不作为线程调度的决策 *[BAI]*/
    SCHAR Priority;							/* 该线程的动态优先级 *[BAI]*/
											/* 0-15普通线程的优先级,16-31实时线程的优先级，无论如何调整，Priority值不会跨区域 *[BAI]*/
    UCHAR EnableStackSwap;					/* 说明本线程的内核栈是否允许被换出到外存中 *[BAI]*/
    volatile UCHAR SwapBusy;				/* 本线程是否正在执行上下文切换，切换时置为TRUE，切换后置为FALSE *[BAI]*/
    BOOLEAN Alerted[MaximumMode];			/* 指明该线程在内核模式和用户模式下是否可以被唤醒 *[BAI]*/
    union {
        LIST_ENTRY WaitListEntry;			/* 当一个线程正在等待被执行时，WaitListEntry作为一个节点加入到某个链表中。eg.KPROCESS 中的 ReadListHead *[BAI] */
        SINGLE_LIST_ENTRY SwapListEntry;
    };

    PRKQUEUE Queue;							/* 若不为NULL，则表示当前线程正在处理此队列对象中的项。内核的队列机制 *[BAI]*/

#if !defined(_AMD64_)

    ULONG WaitTime;							/* 一个线程进入等待时刻的时间点，主要用于平衡集管理器根据一个线程的等待时间来做一些决策 *[BAI]*/
    union {
        struct {
            SHORT KernelApcDisable;			/* 内核普通APC *[BAI]*/
            SHORT SpecialApcDisable;		/* 内核特殊APC *[BAI]*/
        };

        ULONG CombinedApcDisable;			/* 负数值代表该线程不允许插入APC，为0的时候表示可以插入APC *[BAI]*/
    };

#endif

    PVOID Teb;								/* 指向进程地址空间中的一个TEB块。TEB包含了在用户地址空间中需要访问的各种信息。eg.与线程相关的GDI信息，系统异常等 *[BAI]*/
    union {
        KTIMER Timer;						/* 当一个线程在执行过程中需要定时器时.eg.WaitForSingleObject *[BAI]*/
        struct {
...
#define KTHREAD_DISABLE_BOOST_BIT 1
            ...
            union {
                struct {
                    LONG AutoAlignment : 1;	/* 直接继承自所属进程 *[BAI]*/
                    LONG DisableBoost : 1;  /* 直接继承自所属进程 *[BAI]*/
    ...
    union {
        KWAIT_BLOCK WaitBlock[THREAD_WAIT_OBJECTS + 1];					/* 第四项专门用于可等待的定时器对象，前三项可为其他对象 *[BAI]*/
																		/* 因为使用等待在内核中特别频繁，超出四个额外分配空间，此优化非常有意义 *[BAI]*/
        struct {
        ...
        struct {
            ...
            UCHAR LargeStack;                                       /* PsConvertToGuiThread转换为GUI线程时使用 */
        };
    ...
    LIST_ENTRY QueueListEntry;					/* 线程在处理一个队列项时加入到队列对象的线程链表中的节点地址 *[BAI]*/

    //
    // The following fields are accessed during system service dispatch.
    //

    PKTRAP_FRAME TrapFrame;						/* 一个线程最关键的部分，记录控制流状态的数据结构 *[BAI]*/
    PVOID CallbackStack;						/* 该线程的调用栈地址 *[BAI]*/
    PVOID ServiceTable;							/* 该线程使用的系统服务表（KeServiceDescriptorTable) *[BAI]*/
												/* 若这是一个GUI线程，此域指向一个影子系统服务表(KeServiceDescriptorTableShadow) *[BAI]*/
    ...
    UCHAR ApcStateIndex;						/* 在ApcStatePointer中的索引 *[BAI]*/
    UCHAR IdealProcessor                        /* 线程在开始执行时，算出的一个初始值，用来表明期望执行的空闲线程，用作KiProcessorBlock数组中的下标 */;
    BOOLEAN Preempted;							/* 该线程是否在运行过程中被高优先级的线程抢占 *[BAI]*/
    BOOLEAN ProcessReadyQueue;					/* 该线程是否在所属进程的ReadyListHead中 *[BAI]*/
    ...
    BOOLEAN KernelStackResident;					/* 该线程的内核栈是驻留在内存中,换入置为TRUE，换出置为FALSE *[BAI]*/
    SCHAR BasePriority;								/* 静态优先级，从所属进程继承而来,以后可以通过KeSetBasePriorityThread调整 *[BAI]*/
    SCHAR PriorityDecrement;						/* 重新计算线程优先级时的递减值，KeComputeNewPriority计算时会用到 *[BAI]*/
    CHAR Saturation;								/* 说明线程的基本优先级相对与进程的基本优先级的调整量是否超过了整个区间的一般,值为0,1,-1 *[BAI]*/
    KAFFINITY UserAffinity;							/* 该线程的用户亲和性，初始化时也继承自所属进程 *[BAI]*/
    PKPROCESS Process;								/* 线程所属的进程对象,KeInitThread设置 *[BAI]*/
    KAFFINITY Affinity;								/* 该线程对处理器的亲和性，初始时继承自所属进程，通过内核函数KeSetAffnityThread设置 *[BAI]*/
													/* 值有两种设置1.系统亲和性，执行系统任务时通过KeSetSystemAffinityThread设置 *[BAI]*/
													/* 2.用户亲和性，KeRevertToUserAffinityThread设置 *[BAI]*/
    //
    // The below fields are infrequently referenced.
    //
		
    PKAPC_STATE ApcStatePointer[2];					/* 一个指向线程对象的ApcState, 两一个指向线程对象的SavedApcState *[BAI]*/
    union {
        KAPC_STATE SavedApcState;					/* 具体参考APC机制 *[BAI]*/
        struct {
    ...
    PVOID Win32Thread;								/* 指向Windows子系统管理的区域 *[BAI]*/
    PVOID StackBase;								/* 当前线程内核栈的基位置 *[BAI]*/
    union {
        KAPC SuspendApc;							/* 与之下的SuspendSemaphore一起使用，当APC被插入并交付时，KiSuspendThread函数被执行，唤醒结果是在等待SuspendSemaphore上等待（线程的挂起和唤醒通过这一机制实现) *[BAI]*/
        struct {
    ...
    LIST_ENTRY ThreadListEntry;					/* 当一个线程被创建时，他会加入到所属进程对象的ThreadListHead链表中 *[BAI]*/
    PVOID SListFaultAddress;					/* 与用户模式互锁单链表(KeUserPopEntrySListFault函数)有关，记录上一次POP操作发生页面错误的地址,上面的SlistFaultAddress统计发生页面错误的次数 *[BAI]*/
