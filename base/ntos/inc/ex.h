...

{
    ...
    Lookaside = Prcb->PPLookasideList[Number].P;						// 每个处理器上的NonPaged Lookaside List 默认都有两个链表
    ...
    if (Entry == NULL) {												// 若两个链表都没有则尝试从系统链表分配
...
typedef struct _HANDLE_TABLE_ENTRY {
    ...
    union {
        ...
        PVOID Object;                               // 指定句柄所代表的对象

        ULONG ObAttributes;                         // 最低三位有特别含义，参见OBJ_HANDLE_ATTRIBUTES宏定义

        PHANDLE_TABLE_ENTRY_INFO InfoTable;         // 各个句柄表页面的第一个表项，使用此成员指向第一张表
    ...
    union {
        ...
        union {
            ...
            struct {
                ...
                USHORT GrantedAccessIndex;          // 当NtGlobalFlag中包含FLG_KERNEL_STACK_TRACE_OB标记时使用
...
typedef struct _HANDLE_TABLE {
    ...
    ULONG_PTR TableCode;            // 指针指向最顶层的句柄表树节点
    ...
    HANDLE UniqueProcessId;         // 进程ID在callbacks中使用
...
#define HANDLE_TABLE_LOCKS 4
    ...
    EX_PUSH_LOCK HandleTableLock[HANDLE_TABLE_LOCKS];       // 句柄表所，在句柄扩展时使用
    ...
    //
    PHANDLE_TRACE_DEBUG_INFO DebugInfo;     // 调试信息，仅在调试句柄时分配，才有意义
    ...
    union {
        ...
        //
        BOOLEAN StrictFIFO : 1;             // 使用采用FIFO，也即先释放先使用
    };
