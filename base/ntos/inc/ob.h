...
typedef struct _OBJECT_TYPE_INITIALIZER {
    USHORT Length;                                              // 结构本身的长度
    BOOLEAN UseDefaultObject;                                   // 是否使用全局默认的同步对象
    BOOLEAN CaseInsensitive;                                    // 本类对象对象名是否大小写敏感
    ULONG InvalidAttributes;                                    // 本类对象不支持的属性
    GENERIC_MAPPING GenericMapping;                             // ...权限
    ULONG ValidAccessMask;                                      // 本类对像支持的属性集合
    BOOLEAN SecurityRequired;                                   // 本类对象是否需要安全控制
    BOOLEAN MaintainHandleCount;                                // 对象头是否维护句柄统计信息
    BOOLEAN MaintainTypeList;                                   // 对象头是否维护类型类型链表
    POOL_TYPE PoolType;                                         // 本类对象分配的位置
    ULONG DefaultPagedPoolCharge;                               // 对象占用的Page Pool Charge
    ULONG DefaultNonPagedPoolCharge;                            // 对象占用的Non Paged Pool Charge 
    OB_DUMP_METHOD DumpProcedure;
    OB_OPEN_METHOD OpenProcedure;                               // 打开对象时调用的例程
    OB_CLOSE_METHOD CloseProcedure;                             // 关闭对象时调用的例程
    OB_DELETE_METHOD DeleteProcedure;                           // 删除对象时调用的例程
    OB_PARSE_METHOD ParseProcedure;                             // 对象路径解析函数
    OB_SECURITY_METHOD SecurityProcedure;                       // 查询设置对象的安全描述符函数
    OB_QUERYNAME_METHOD QueryNameProcedure;                     // 查询名字函数
    OB_OKAYTOCLOSE_METHOD OkayToCloseProcedure;                 // 每次关闭句柄前都会调用这个函数检查是否可关闭
...
{
    ERESOURCE Mutex;
    LIST_ENTRY TypeList;                    // 本类对象链表，记录所有同类的对象
    UNICODE_STRING Name;                    // Copy from object header for convenience;类型名
    PVOID DefaultObject;                    // 指本类对象默认使用的同步事件对象
    ULONG Index;                            // 此对象类型在全局对象类型数组中的索引 ObpObjectTypes
    ULONG TotalNumberOfObjects;             // 对象列表中总的对象个数
    ULONG TotalNumberOfHandles;             // 所有同类对象的打开句柄总数
    ULONG HighWaterNumberOfObjects;         // 历史本类对象个数的峰值
    ULONG HighWaterNumberOfHandles;         // 历史本类对像打开句柄总数的峰值
    OBJECT_TYPE_INITIALIZER TypeInfo;       // 对象类型信息
#ifdef POOL_TAGGING
    ULONG Key;                              // 事实上用作分配内存的tag, 同类对象占用的内存都标记为同一个tag
#endif //POOL_TAGGING
...
typedef struct _OBJECT_HEADER 
{
    ...
    UCHAR NameInfoOffset;                       // 名称信息的偏移
    UCHAR HandleInfoOffset;                     // 句柄信息的偏移
    UCHAR QuotaInfoOffset;                      // 配额信息的偏移(当不等于该类对象的默认大小时要用到这个)
    UCHAR Flags;                                // 对象的一些属性标志

    union {
        POBJECT_CREATE_INFORMATION ObjectCreateInfo;    // 来源于创建对象的OBJECT_ATTRIBUTES
        PVOID QuotaBlockCharged;
    };

    PSECURITY_DESCRIPTOR SecurityDescriptor;    // 该对象的安全描述符(对象的拥有者，ACL等)
    QUAD Body;                                  // 通用对象头后面紧跟着具体对象类型的结构定义（该字段是后面真正结构体中的第一个成员）
...
typedef struct _OBJECT_HEADER_NAME_INFO {
    POBJECT_DIRECTORY Directory;                    // 对象目录
...
typedef struct _OBJECT_HEADER_CREATOR_INFO {
    LIST_ENTRY TypeList;                            // 用来挂入所属的"对象类型"中的链表
    HANDLE CreatorUniqueProcess;                    // 该对象是由哪个进程创建的
