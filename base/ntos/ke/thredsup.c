...
Routine Description:
    ...
    该函数使一个线程准备被调度，同时尝试派发该线程执行，通过将其分配到一个空闲的处理器上，或者是抢占其他优先级较低的线程
    ...
    如果线程可以被分配到一个空闲的处理器，则该线程进入到standby状态，之后目标处理器在下一次idle loop时会切换到该线程执行
    ...
    如果一个低优先级的线程可以被抢占，之后该线程进入到standby状态，目标处理器被请求去派发。
    ...
    如果该线程无法被分配到一个空闲的处理器上或者是另一个线程无法被抢占，则该线程被插入到Dispatch Ready list 的头或者尾，根据其优先级来选择

    N.B. This function is called at SYNCH level with no PRCB locks held.
    该函数在SYNCH Level，同时不占有PRCB locks
    N.B. This function may be called with the dispatcher database lock held.
    该函数可能被调用，同时dispatcher database lock被占有的情况下
    N.B. Neither the priority nor the affinity of a thread in the deferred
         ...
         processor.
    每一个处理器不占有PRCB lock的情况下，在defereed read state状态下的线程的优先级和亲缘性不能被更改。
Arguments:
...

{
    ...
    ASSERT(Thread->State == DeferredReady);                                     // 断言该处理器的状态一定是DeferredReady

    ASSERT((Thread->Priority >= 0) && (Thread->Priority <= HIGH_PRIORITY));     // 断言其的优先级在有效范围内

    //
    // Check if a priority adjustment is requested.
    //
    /* 通过该域来判断是否一个线程优先级调整被请求了 */
    ...
    else if (Thread->AdjustReason == AdjustBoost)                           // 调整原因是调整增加
    {
        ...
        //
        // 由于增加优先级的事件发生， 优先级被调整。
        // 当前线程的优先级被存储在线程对象的 adjust increment
        // 获取线程锁
        // 如果等待线程的优先级小于等于当前线程的优先级，同时等待线程的优先级小于the time critical priority bound,同时
        // 见下面代码，说不太明白
        // 这个增加的发生在当前执行时限的结束。
            ...
            (Thread->DisableBoost == FALSE)) 
        {
            ...
            Priority = min(Thread->AdjustIncrement + 1,
                           TIME_CRITICAL_PRIORITY_BOUND - 1);                   // 当前线程增加+1和Time critical priority bound-1的最小值

            ASSERT((Thread->PriorityDecrement >= 0) &&
                   (Thread->PriorityDecrement <= Thread->Priority));            // 断言减小值的范围

            Thread->PriorityDecrement += ((SCHAR)Priority - Thread->Priority);  // 线程优先级减小值,为了Compute 时，用来减少，平衡线程调度

            ASSERT((Thread->PriorityDecrement >= 0) &&
                   (Thread->PriorityDecrement <= Priority));                    // 断言

            Thread->Priority = (SCHAR)Priority;                                 // 设置线程优先级
        }
        ...
        if (Thread->Quantum < LOCK_OWNERSHIP_QUANTUM)                           // 确保线程的时限正确的对应lock ownership 和 charge 时限
        {
        ...
        Thread->AdjustReason = AdjustNone;                                      // 更新调整原因

    } 
    else if (Thread->AdjustReason == AdjustUnwait)                              // 调整原因是调整不等待
    {
        ...
        //
        // 由于一个不等待的操作，发生优先级的调整
        // 优先级的增量存储在线程对象的adjust increment 成员中
        // 获取线程锁
        // 如果线程运行在实时级别，则重设线程时限。否则，计算下一个线程的优先级，同时charge 该线程
        ...
        if (Thread->Priority < LOW_REALTIME_PRIORITY)
        {
            ...
            if (Thread->BasePriority >= TIME_CRITICAL_PRIORITY_BOUND)          // 如果线程的BasePrioriy 大于Time critical priority bound,则重新初始化线程的时限
            {
            ...
            else
            {
                ...
                // quantum.
                //
                // 如果该线程并没有收到一个unsual boost,同时优先级增量非0，则线程重新初始化线程的时限
                ...
                //
                // 如果一个线程正处于执行一个Kernel APC的状态，则不改变当前线程的时限。
                // 等待的代码会在Kernel APC执行完之后，同时等待被满足，才会charge quantum. 否则，减少线程的quantum,同时如果时限完成后，重新计算线程的优先级
            ...
            // 如果线程不是运行在unusual boost状态，同时boosts并没有被禁止，则尝试增加指定的增量
                ...
                (Thread->DisableBoost == FALSE)) 
            {
                ...
                //
                // 如果指定的线程是来自一有foreground memory priority的进程，则增加foreground boost .
                ...
                //
                // 如果调整的优先级大于当前优先级，则调整，但是不能高于最低实时优先级
                if (Priority > Thread->Priority) 
                {
                    ...
                    // quantum.
                    //
                    // 如果新线程的优先级大于该线程的base priority+指定的增量，则设置priority decrement为一个时限后，减少这个增量
                    ...
                    Thread->Priority = (SCHAR)Priority;                 // 调整优先级
                }
            }
    ...
    // 如果有一个空闲的处理器，则根据如下情况安排线程到空闲的处理器上:
    ...
    //      如果一个线程有一个soft(指拥有一个优先的Affinity set),同时set包含一个空闲的处理器，则reduce the set to the intersection of two sets
...
IdleAssignment:
    ...
    do 
    {
        ...
        if (IdleSet != 0)                                       // 它期望执行的处理器处于空闲状态
        {
            if ((IdleSet & AFFINITY_MASK(Processor)) == 0)      // 说明不是线程初始化时计算出来的那个空闲的处理器
            {
                ...
                //
                // 如果该线程可以运行的下一个处理器，包含在这个idle set中，则选择该处理器来运行
                IdealProcessor = Processor;
                Processor = Thread->NextProcessor;
                if ((IdleSet & AFFINITY_MASK(Processor)) == 0)          // Next Processor 也不在idle set中
                {
                    ...
                    if ((IdleSet & AFFINITY_MASK(Processor)) == 0)      // 尝试匹配当前处理器
                    {
                        ...
                        KeFindFirstSetLeftAffinity(IdleSet, &Processor);    // 当前处理器中也没有合适的，则调用该函数继续查找处理器
                    }
                }
            }
            ...
            //
            // 获取当前和目标PRCB 锁，确保被选择的处理器任然处于idle状态， 线程可以运行在处理器上
                ...
                ((Thread->Affinity & TargetPrcb->SetMember) != 0)) 
            {
                ...
                //
                // 设置线程状态为standby, 设置在该处理器上运行线程，同时清除idle summary中的标志
                Thread->State = Standby;
    ...
    /* 以上是有空闲处理器的时候，选择选择处理器和设置处理器执行的新线程， */
    /* 以下是没有空闲处理器的时候，抢占的过程 */
    //
    ...
    //
    /* 没有合适的空闲处理器来运行该线程，获取当前和目标 PRCB locks,确保目标处理器不是空闲，同该线程依然可以运行在该处理器上 */
    KiAcquireTwoPrcbLocks(CurrentPrcb, TargetPrcb);
    ThreadPriority = Thread->Priority;
...
Routine Description:
    ...
    该函数用来处理该PRCB的deferred ready list, 工作在SYNCH level, 该函数只有当deferred ready list不为空时调用
    deferred ready list是每一个处理器一个链表，同时itemes 只是插入和移除单独的处理器，因此不需要同步
Arguments:
...
Routine Description:
    ...
    1. 如果该线程的进程不在内存中，则插入该线程到进程的ready list中。
    2. 如果该线程的内核栈不在内存中，则插入该线程到kernel stack in swap list中。
    3. 或者插入该线程到延迟就绪链表中(deferred ready list).
    N.B. This function is called with the dispatcher database lock held and
         returns with the lock held.
    该函数被调用时，应该获取到了dispatcher database 锁， 同时返回时也应该保持锁状态
    N.B. The deferred ready list is a per processor list and items are
         only inserted and removed from the respective processor. Thus
         no synchronization of the list is required.
    延迟就绪链表时每一个处理器的链表，这些项都分别从各自的处理器上插入和删除。因此无需做同步处理
Arguments:

    Thread - Supplies a pointer to a dispatcher object of type thread.
    指向一个线程的dispatcher object header.
Return Value:
...

{
    ...
    //
    /* 如果线程不在内存中，插入线程到进程的ready queue ,将进程换入 */
    Process = Thread->ApcState.Process;                                     // 这个是获取到最真实的进程，因为该线程可能是注入到其他进程的线程
    if ( Process->State != ProcessInMemory )
    {
        Thread->State = Ready;
        Thread->ProcessReadyQueue = TRUE;
        InsertTailList(&Process->ReadyListHead, &Thread->WaitListEntry);    // 插入到进程的ReadList, 使用的是WaitListEntry
        if ( Process->State == ProcessOutOfMemory )                         // 开始调度进程，进入内存
        {
            Process->State = ProcessInTransition;                           // 更新进程状态
            InterlockedPushEntrySingleList(&KiProcessInSwapListHead,        // 同时把该进程插入到全局进程的KiProcessInSwapList中
    ...
    else if (Thread->KernelStackResident == FALSE)                          // 如果该线程的内核栈不在内存中
    {
        ...
        Process->StackCount += 1;                                           // 增加进程的内核栈计数

        ASSERT(Thread->State != Transition);

        Thread->State = Transition;                                         // 更新线程状态
        InterlockedPushEntrySingleList(&KiStackInSwapListHead,              // 插入该线程到KiStackSwapListHead中
                                       &Thread->SwapListEntry);

        KiSetInternalEvent(&KiSwapEvent, KiSwappingThread);                 // 设置信号
        return;
    ...
    else 
    {
        ...
        //
        /* 插入线程到deferred ready list */
        KiInsertDeferredReadyList(Thread);                                  // 否则插入到延迟就绪链表中
        return;
    }
}
...
Routine Description:
    ...
    N.B. This function is called with the specified PRCB lock held and also
         returns with the lock held.
   该函数选则一个线程运行到指定的处理器上。该函数执行时，PRCB应该已经上锁，同时返回时依然带锁
Arguments:
...

{
    ...
    // queues.
    //
    // 找到一个从指定的PRCB的Ready queue中选取一个read状态的线程
    if ((Thread = KiSelectReadyThread(0, Prcb)) == NULL) 
    {
        ...
        //
        // 如果该PRCB结构中没哟Ready thread, 则选则该Prcb的IdleThread, 则更新该处理器在KiIdleSumary结构中的位。
        // 选则一个idle thread 避免了做一次从dispatcher queue中查找适合执行的线程的操作。idle将实现 一个完整的查找
...
Routine Description:
    ...
    这个函数设置指定线程的Afinity为一个新值。如果新值不是一个所属进程的affinity 的子集或者是空，则Bugchek
    如果指定的线程正在运行，或者将要去运行，则它没法再继续运行了，会安排目标处理器重新调度。
    如果指定的线程处于一个准备状态，同时不是在期望处理器的read queue中，则会重新ready, 重新评估它要运行的处理器
Arguments:
...

{
    ...
    //
    // 如果该线程的user ideal 处理器不是 新Affinity的一个成员，则重新计算user ideal processor
#if !defined(NT_UP)

    if ((Affinity & AFFINITY_MASK(Thread->UserIdealProcessor)) == 0) 
    {
        if (KeNumberNodes > 1)              // 多节点考虑
        {
    ...
    /* 以下是要更新线程的Affinity,当然根据当前线程不同的状态来更新 */
    //
    ...
    //
    // 如果当前线程不是带着状态SystemAffinity为 Active下运行，则设置当前线程的Affintiy, 同时切换当前线程的转态
    if (Thread->SystemAffinityActive == FALSE) 
    {
        ...
        do 
        {
            ...
            case Ready:
                if (Thread->ProcessReadyQueue == FALSE)                 // 如果不在当前处理器的Ready Queue链表中
                {
                    Processor = Thread->NextProcessor;
...

{
    ...
    // event.
    //
    /* 如果swap event wait queue不为空，则解除该swap thread的等待状态。否则，设置设置swap event有信号 */
...

{
    ...
    {
        KiProcessDeferredReadyList(CurrentPrcb);                            // 首先从处理当前处理器的Deferred Ready List
    }
    ...
    KiAcquirePrcbLock(CurrentPrcb);                                         // 获取当前处理器锁
    ...
    else 
    {
        ...
        if ((NewThread = KiSelectReadyThread(0, CurrentPrcb)) != NULL)      // 当前处理器上找下一个线程
        {
            CurrentPrcb->CurrentThread = NewThread;
            NewThread->State = Running;
        ...
        else 
        {
            ...
            KiSetIdleSummary(CurrentPrcb->SetMember);                       // 设置处理器空闲
            ...
            KiReleasePrcbLock(CurrentPrcb);                                   // 释放掉当前处理器锁，开始在其他处理器上找
            Processor = CurrentPrcb->Number;
            Index = Processor;
            if (KeNumberNodes > 1) 
            {
                KeFindFirstSetLeftAffinity(CurrentPrcb->ParentNode->ProcessorMask,
                                           (PULONG)&Index);                   // 优先查找同一个节点上的其他处理器
            }
            ...
            do 
            {
                ...
                {
                    if (TargetPrcb->ReadySummary != 0)                        // 确认有优先级链表
                    {
                        ...
                        if ((NewThread = CurrentPrcb->NextThread) == NULL)          // 如果当前处理器的NextThread是空
                        {
                            if ((TargetPrcb->ReadySummary != 0) &&
                                (NewThread = KiFindReadyThread(Processor,
                                                               TargetPrcb)) != NULL) // 从目标处理器上找到一个线程取出
                            {
                                ...
                                NewThread->State = Running;                         // 调度到当前处理器上执行
                        ...
                        else 
                        {
                            ...
                            if (NewThread == CurrentPrcb->IdleThread)             // 当前处理器已经被选择执行了一个线程，假如是空闲线程
                            {
                                CurrentPrcb->NextThread = NULL;
