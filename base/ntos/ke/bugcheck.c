...

{
    ...
    RtlCaptureContext(&KeGetCurrentPrcb()->ProcessorState.ContextFrame);                        /* 保存寄存器状态到PRCB->ProcessorState结构中 */
    KiSaveProcessorControlState(&KeGetCurrentPrcb()->ProcessorState);                           /* 填充ProcessState中SpecialRegister结构 */
    ...
    ContextSave = KeGetCurrentPrcb()->ProcessorState.ContextFrame;                              /* 保存到局部变量中 */
    ...
    KiBugCheckData[0] = BugCheckCode;                           /* 保存BugCheck数据到全局变量中 */
    ...
    if (KiBugCheckDriver)
    {
        KeBugCheckUnicodeToAnsi(KiBugCheckDriver,                   /* 转换BugCheckDriver的字符为Ansi */
                                AnsiBuffer,
                                sizeof(AnsiBuffer));
    }
    else 
    ...
    if (KdPitchDebugger == FALSE )                                  /* 否Pitch 给Debugger */
    {
    ...
    if ((BugCheckCode != MANUALLY_INITIATED_CRASH) &&                           /* 非手动崩溃和内核调试支持*/
        (KdDebuggerEnabled)) 
    {
