...

{
    ...
    /* 提升到SYNCH_LEVLEL级别（IPI-2 = 27 ),同时获取进程锁 */
    ...
    //
    /* 算出一个新的ideal processor，更新进程的ThreadSeed值 */
...

{
    ...
    CurrentThread = KeGetCurrentThread();
    KiLockDispatcherDatabase(&OldIrql);                                     // 拿锁，提升权限
    ...
    // processor, then select a new thread for the current processor.
    //
    // 如果当前处理器不在新的Affinity中，同时另一个线程尚未被选择到当前处理器上执行，则给当前处理器选则一个新的线程
    Prcb = KeGetCurrentPrcb();
    if ((Prcb->SetMember & CurrentThread->Affinity) == 0) 
    {
        KiAcquirePrcbLock(Prcb);                                    // 拿到PRCB锁
        if (Prcb->NextThread == NULL) 
        {
            NewThread = KiSelectNextThread(Prcb);                   // 选则新线程
            NewThread->State = Standby;
            Prcb->NextThread = NewThread;
        }
