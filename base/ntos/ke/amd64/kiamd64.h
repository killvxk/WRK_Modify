...

{
...
#if !defined(NT_UP)					// NT_UP whether your driver run at uniprocessor or multiprocessor ,单处理器还是多处理器上
    ...
    InterlockedXor64((LONG64 volatile *)&OldProcess->ActiveProcessors, SetMember);				// ActiveProcess 表明当前进程正在哪些处理器上运行;清楚处理器标志位在旧的进程KPROCESS上
    ...
    InterlockedXor64((LONG64 volatile *)&NewProcess->ActiveProcessors, SetMember);				// 设置处理器标志位在新的进程KPROCESS上
