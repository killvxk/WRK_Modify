...
/* 判断是否可警醒的条件:
    如果Alertable = TRUE:
        1. 处理器模式 == Wait mode, thread 被警醒， 则等待完成，直接返回，状态status = ALERTED.
        2. 如果 Wait mode == user mode, user APC queue非空， 设置user APC pending, 则等待完成，直接返回， staus = USER_APC
        3. 如果线程被内核模式警醒，则等待完成，直接返回，staus = ALERTED.
    如果Alertable = FALSE:
        1. 如果 wait mode = user mode, 同时有APC pending, 则等待完成，直接返回， status = USER_APC
*/
...

{
    ...
    //
    /* 开始等待循环 */
    /* 注意，如果一个Kernel APC 在等待的过程中被派发，或者是第一次循环时Kernel APC is pending, 则this loop is repeated */
    do {
        ...
        // N.B. This can only happen in a multiprocessor system.
        //
        /* 测试是否一个Kernel APC is pending */
        /* 如果一个Kernel APC is pending, 且特殊APC disable count == 0, 同时Previous IRQL 小于 APC_LEVEL，之后一个Kernel APC 被另一个处理器插入，当IRQL提升到 DISPATCH_LEVEL */
        /* 但是在这之前 dispatcher database was locked，这是在多处理器下才可能发生的情况 */
        Thread->Preempted = FALSE;
        if ( Thread->ApcState.KernelApcPending &&
            (Thread->SpecialApcDisable == 0) &&
            (Thread->WaitIrql < APC_LEVEL) ) 
        {
            ...
            // in the delivery of the kernel APC if possible.
            //
            /* Unlock dispatcher databas, 同时降低IRQL到之前的值。 一个APC中断将立即产生，用来派发Kernel APC */
            KiUnlockDispatcherDatabase(Thread->WaitIrql);
        ...
        else 
        {
            ...
            //
            /* 没有APC Pending的情况 */
            /* 判断一个对象是否是mutant object, 而且Mutant object 已经被递归MINLONG 次，则产生一个异常。*/
            /* 否则，如果一个mutant的信号状态大于0 ，切当前线程是mutant的owner,则满足等待 */
            ASSERT(Objectx->Header.Type != QueueObject);
