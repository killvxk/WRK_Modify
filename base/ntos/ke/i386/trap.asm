...
;
; Sanitize the segment registers /* 初始化内核状态下的FS，DS, ES 段寄存器*/
;
        mov     ecx, KGDT_R0_PCR
        mov     fs, ecx
...
; shift to the threads stack before enabling interrupts./* 从PCR中获取内核栈的地址初始化，首先获取到TSS，之后获取ESP0；然后保存将用户态下的SS和ESP压栈 */
;
        mov     ecx, PCR[PcTss]        ;
        mov     esp, [ecx]+TssEsp0
...
.errnz (EFLAGS_TF AND 0FFFF00FFh)
        or      byte ptr [esp+1], EFLAGS_TF/0100h  ; Set TF flag ready for return or    设置好TF标志位
...
;
        ...
        pushfd
Kfsc10:
        push    2                           ; Sanitize eflags, clear direction, NT etc 压入EFLAGS？
        add     edx, 8                      ; (edx) -> arguments 使EDX指向参数信息？
        popfd                               ;
.errnz(EFLAGS_INTERRUPT_MASK AND 0FFFF00FFh)
        or      byte ptr [esp+1], EFLAGS_INTERRUPT_MASK/0100h ; Enable interrupts in eflags  设置IF标志位

        push    KGDT_R3_CODE OR RPL_MASK    ; Push user CS  压入用户态的CS段
        push    dword ptr ds:[USER_SHARED_DATA+UsSystemCallReturn] ; push return address  // 压入返回地址
        push    0                           ; put pad dword for error on stack // 压入一个错误号
        push    ebp                         ; save the non-volatile registers // 压入一些非易失性寄存器的值
        push    ebx                         ;
        push    esi                         ;
        push    edi                         ;
        mov     ebx, PCR[PcSelfPcr]         ; Get PRCB address  EBX存放当前PCR的值
        push    KGDT_R3_TEB OR RPL_MASK     ; Push user mode FS 压入用户态的FS值，即TEB的地址
        mov     esi, [ebx].PcPrcbData+PbCurrentThread   ; get current thread address    ESI 保存获取到当前线程的值
;
; Save the old exception list in trap frame and initialize a new empty
; exception list.
;

        push    [ebx].PcExceptionList       ; save old exception list // 继续压入PCR旧的 exception list 
        mov     [ebx].PcExceptionList, EXCEPTION_CHAIN_END ; set new empty list // 设置为空
        mov     ebp, [esi].ThInitialStack                       // EBP 保存当前线程栈

;
; Save the old previous mode in trap frame, allocate remainder of trap frame,
; and set the new previous mode.
;
        push    MODE_MASK                  ; Save previous mode as user     // 压入PreviousMode
        sub     esp,TsPreviousPreviousMode ; allocate remainder of trap frame           // 在栈上保留出一段空间
        sub     ebp, NPX_FRAME_LENGTH + KTRAP_FRAME_LENGTH
        mov     byte ptr [esi].ThPreviousMode,MODE_MASK ; set new previous mode of user  // 设置好新的PreviousMode
;
...
;
        ...
        cmp     ebp, esp
        jne     short Kfsc91                    ; 意思是如果不是VX86模式，这两个值一定相同？
...
;
        ...
        mov     [esi].ThTrapFrame, ebp   ; set new trap frame address       设置好线程上的TrapFrame值

        jnz     Dr_FastCallDrSave       ; if nz, debugging is active on thread 如果不相同，说明正在调试
