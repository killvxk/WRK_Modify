...

{
    ...
    KeContextFromKframes(TrapFrame, ExceptionFrame, &ContextFrame);                         // 构造context 
    ...
    {        
        ...
        UserStack = ((TopOfStack - sizeof (EXCEPTION_REGISTRATION_RECORD)) & ~CONTEXT_ROUND) -              // 在用户栈上预留了一段EXCEPTION_REGISTRATION_RECORD, CONTEXT_RECORD, KAPC_RECORD大小空间
            Length;
        ...
        RtlCopyMemory ((PULONG)(UserStack + sizeof(KAPC_RECORD)),                                           // 将Context数据拷贝到用户栈上
        ...
        TrapFrame->SegCs = SANITIZE_SEG(KGDT_R3_CODE, UserMode);                                            // 强制更新TrapFrame中的所有选择子为用户选择子，和 EFlags标志
        ...
        if (KeGetCurrentThread()->Iopl)                                                                     // IOPL作什么用的
        {
        ...
        TrapFrame->HardwareEsp = UserStack;                                                                 // 更新TrapFrame中数据
