...
Routine Description:
    ...
    N.B. This function does not check the accessibility of the context record.
         ...
         as appropriate.
    该函数初始化一个线程对象及其相关的内容，该函数不对context record内容的可访问性进行检查。它假使调用者有权限，或者做好了异常处理
Arguments:
    ...
    StartRoutine - Supplies an optional pointer to a function that is to be
        ...
        指向一个可选的函数指针，当系统完成对线程的初始化。 如果该线程时隔系统线程，参数被指定，并且完成执行在内核模式下
    StartContext - Supplies an optional pointer to an arbitrary data structure
        ...
        parameter is specified if the thread is a system thread and will
        execute totally in kernel mode.
        只有当该线程是一个系统线程，且该函数完全执行在内核模式下。 指向一个任意的数据结构
    ContextFrame - Supplies an optional pointer a context frame which contains
        ...
        可选参数，指向一个用来初始化用户模式现成状态的context结构。 
Return Value:
...

{
    ...
    //
    /* 如果ContextFrame 有效，则代表是用户线程创建，则初始化一个trap frame和一个 exception frame  */
    if (ARGUMENT_PRESENT(ContextFrame)) 
    {
        ...
        //
        // The 80387 save area is at the very base of the kernel stack.         // 用户模式线程会开辟如下两个结构
        //
        ...
        //
        /* 暂不了解有何用处，初始化Context中的结构 */
        if (KeI386FxsrPresent == TRUE)                                          // 不知道这个全局变量是控制什么的
        {
