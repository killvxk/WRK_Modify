...

{
    ...
    Thread->State = DeferredReady;                      // 单核要设置该线程状态为DeferedRead状态，同时调用KiDeferredReadyThread
    Thread->DeferredProcessor = 0;
    KiDeferredReadyThread(Thread);

#else

    PKPRCB Prcb;                                        // 如果是多核，插入该线程到该处理器的PRCB结构中的DeferedReadList中，同时直接设置为Ready状态
...
Routine Description:
    ...
    该函数查找dispatch ready queue, 从最低优先级到最高优先级，找一个可以在指定处理器上运行的线程
Arguments:
