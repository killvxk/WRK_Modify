...
#error "no target architecture"
        ...
        /* 初始化线程，线程的执行点是PspUserThreadStartup */
        if (NT_SUCCESS (Status)) 
        {
    ...
    /* 如果该进程创建之前的线程数是0，说明这是在初创建的进程在创建第一个线程，依次调用进程创建回调函数 */
    ...
    // in time
    //
    /* 如果该进程加入了一个Job,同时completion port 有效则report */
    Job = Process->Job;
...

{
    ...
    if (ARGUMENT_PRESENT (ParentProcess)) {
        Status = PspCreateProcess (ProcessHandle,                                       // 直接调用了PspCreateProcess
                                   DesiredAccess,
                                   ObjectAttributes,
                                   ParentProcess,
                                   Flags,
...

{
    ...
    // Section
    //
    /* 如果指定了SectionHandle,则使用该SectionHandle来创建 */
    ...
    // DebugPort
    //
    /* 如果指定了调试端口对象，则使用该调试端口对象，若没有，则继承父调试端口对象 */
    ...
    // ExceptionPort
    //
    /* 如果指定了，则设为指定的异常端口对象，若没有指定，则为空，这和DebugPort, SectionHandle还不太一样 */
    ...
    // ObInitSystem.
    //
    /* 如果父进程不为空，则克隆父进程的对象表，否则使用ObInitSystem的对象表，同时初始化进程地址空间 */
    if (Parent != NULL) 
    {
    ...
    //
    /* 初始化安全属性，父进程可能为空（仅有一次，也就是系统启动时），系统启动后，总是有parent的，这样我们可以复制安全属性到新进程中 */
    ...
    //
    /* 初始化进程地址空间，4中可能性：Boot Process. System Process. User Process( Clone Address Space ). User Process( New Image Address Space) */
    ...
    else if (Parent != NULL) 
    {
        ...
        else 
        {
            ...
            //
            /* 系统进程的情况。没有指定要克隆的进程和要映射的section */
    ...
    //
    /* 判断父的job,若有效，则引用该job,同时将process加入 */
    if (Parent) 
    {
        Job = Parent->Job;
    ...
    // rundown.
    //
    /* 一旦将该进程插入到对象链表中，则外部就可以使用名字来访问了。但是OPEN by ID 还不行 */
