...

{
    ...
    CurrentThread = PsGetCurrentThread ();
    KeEnterCriticalRegionThread (&CurrentThread->Tcb);					// 禁用当前线程的APC
    ...
    if (CidEntry != NULL) {
        ...
        ExUnlockHandleTableEntry(PspCidTable, CidEntry);			// 查找过程有加锁的过程,此刻需要解锁
    }

    KeLeaveCriticalRegionThread (&CurrentThread->Tcb);				// 开启当前线程的APC
    return Status;
}
