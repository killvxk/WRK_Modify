...

{
    ...
    ObjectCreateInfo = ObpAllocateObjectCreateInfoBuffer();							// 默认从PRCB的Nonpage Lookasid List上分配，ObjectCreateInfo默认在CreateInof List上
    ...
    else 
    {
        ...
        if (NT_SUCCESS(Status)) 
        {
            ...
            if (ObjectType->TypeInfo.InvalidAttributes & ObjectCreateInfo->Attributes) 				// 类型信息中不支持的属性若包含，则错误
            {
                ...
                if (PagedPoolCharge == 0) {																// 默认PagedPoolCharge， charge不知怎么理解？？？
                ...
                if (NT_SUCCESS(Status)) {
                    ...
                    *Object = &ObjectHeader->Body;
                    ...
                    if (ObjectHeader->Flags & OB_FLAG_PERMANENT_OBJECT) {				// 若指定了永久权限，则检查之前的模式是否有权限
...

{
    ...
    try {
        ...
        if (ARGUMENT_PRESENT(ObjectAttributes)) {
            ...
            if (ProbeMode != KernelMode) {													// 用户地址检测数据是否可读
            ...
            if (ObjectAttributes->Length != sizeof(OBJECT_ATTRIBUTES) ||
                (ObjectAttributes->Attributes & ~OBJ_ALL_VALID_ATTRIBUTES)) {				// 检测OA
            ...
            ObjectCreateInfo->RootDirectory = ObjectAttributes->RootDirectory;				// 根目录对象类型地址
            ObjectCreateInfo->Attributes = ObjectAttributes->Attributes & OBJ_ALL_VALID_ATTRIBUTES;		// 取出所有有效位
            //
            // Remove privileged option if passed in from user mode
            //
            if (CreatorMode != KernelMode) {
                ObjectCreateInfo->Attributes &= ~OBJ_KERNEL_HANDLE;							// 如果是从用户态传来的，取消OBJ_KENEL_HANDLE标志位
            } else if (ObWatchHandles) {													// 调试用
                if ((ObjectCreateInfo->Attributes&OBJ_KERNEL_HANDLE) == 0 &&
                    PsGetCurrentProcess() != PsInitialSystemProcess) {
                    DbgBreakPoint ();
                }
            }
            ObjectName = ObjectAttributes->ObjectName;										// 直接赋值
            SecurityDescriptor = ObjectAttributes->SecurityDescriptor;
            SecurityQos = ObjectAttributes->SecurityQualityOfService;

            if (ARGUMENT_PRESENT(SecurityDescriptor)) {										// 如果有安全描述符的话，执行安全组件中的获取安全描述符函数，未看这个流程
            ...
            if (ARGUMENT_PRESENT(SecurityQos)) {														// 若指定了SecurityQos，则赋值

                if (ProbeMode != KernelMode) {

                    ProbeForReadSmallStructure( SecurityQos, sizeof(*SecurityQos), sizeof(ULONG));		// 比普通的校验多个按字节对齐校验
                }
        ...
        Status = GetExceptionCode();																// 返回值是NT_STAUTS类型
...

{
    ...
    /* 默认从NonPagedPool 分配 空间，若指定了PagedPool则从PagedPool 分配 */
