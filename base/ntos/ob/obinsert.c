...

{
    ...
    ObjectHeader = OBJECT_TO_OBJECT_HEADER(Object);                         // 获取对象头
    ...
    ObjectCreateInfo = ObjectHeader->ObjectCreateInfo;                      // 对象头创建信息

    ObjectType = ObjectHeader->Type;                                        // 对象类型

    NameInfo = ObpReferenceNameInfo( ObjectHeader );                        // 对象名信息
    ...
    /* 如果安全检查不是必须的，同时一个对象名没有指定，则差入一个未命名对象， */
    ...
    //
    /* 对象要么是命名的，或者需要完全安全检查，如果调用者并没有指定一个access state,则使用请求的access创建一个 */
    ...
    if (ObjectName != NULL) {
        ...
        /* 我们找到了名字，同时它并不是我们之前输入的对象。所以，我们不能再插入一次，返回合适的状态值 */
    ...
    /* 如果我们在创建一个新的对象，我们需要赋值安全信息给它。AccessState结构中有一个指针指向调用者指定的安全描述符信息.对象头中的安全描述符域
    必须指向最终的安全描述符，或者是NULL（如果该对象没有指定安全信息）*/
    if (InsertObject == Object) {
        ...
        /* 只有如下的对象有安全描述符: 命名对象，未命名的对象，但是对象类型显示指明了需要一个安全描述符 */
