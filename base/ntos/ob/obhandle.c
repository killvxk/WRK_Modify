...

{
    ...
    //
    if (Attributes & OBJ_FORCE_ACCESS_CHECK) {                                              // 如果指定了强制安全检查位，则指定访问模式为UserMode.否则为KernelMode
    ...
    try {
        ...
        Status = ObpChargeQuotaForObject( ObjectHeader, ObjectType, &NewObject );             // 指定用户配额
        ...
        if (Attributes & OBJ_EXCLUSIVE) {                                                     // 如果Caller指定了严格检查，则确定attribute和头 Flags匹配正确
...

{
    ...
    ObpValidateIrql( "ObpCreateHandle" );                                               // 检查IRQL
    ...
    ObjectHeader = OBJECT_TO_OBJECT_HEADER( Object );
    ObjectType = ObjectHeader->Type;                                                    // 获取对象头和对象类型
    ...
    if ((ARGUMENT_PRESENT( ExpectedObjectType )) &&                                     // 若参数指定的对象类型和该对象头的类型不一致返回。
    ...
    ObjectTableEntry.Object = ObjectHeader;                                             // 保存对象头
    ...
    {                                                                                   // 若是在内核句柄下，ObjectTable为ObpKernelHandleTable
    ...
    else 
    {
        ...
        ObjectTable = ObpGetObjectTable();                                              // 默认情况下，则ObjectTable为EPROCESS->ObjectTable，即当前进程的句柄表
    }
    ...
    Status = ObpIncrementHandleCount( OpenReason,                                       // 检查是否可以独占打开，检查权限，若各项检查通过才可打开对象，递增句柄计数，调用对象的OpenProcedure等等工作
                                      PsGetCurrentProcess(),
                                      Object,
                                      ObjectType,
                                      AccessState,
