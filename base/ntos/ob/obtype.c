...

{
    ...
    /* 查看类型名是否已经存在于\ObjectTypes目录，若存在返回错误；否则增加 */
    ...
    /* 其实现是根据指定的创建信息，分配合适的额外头空间，并填充对应的信息，返回 */
    ...
    /* 如果没有类型对象类型，则这个就是。类型对象类型一定是第一个创建的对象类型 */
    ...
    /* 默认的类型中指定的默认Quota信息，就是这几个默认头的大小 */
