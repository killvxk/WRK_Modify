...

{
    ...
    if (Attributes & OBJ_CASE_INSENSITIVE) {                         // 设置大小写敏感
    ...
    Buffer = Name->Buffer;                                            // 名字Buffer和长度
    ...
    HashValue = HashIndex;
    HashIndex %= NUMBER_HASH_BUCKETS;                                  // HashValue算法
    ...
    while (1) {                                                         // 遍历列表，比较名字
        ...
        if (DirectoryEntry) {
            ...
            /* 返回从DeviceMap指定的 GlobalDosDevicesDirectory */
